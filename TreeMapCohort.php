  ...
  <head>
    ...
    <script type="text/javascript">
		...
		function drawChart() {
			...
			var options = {
				title: 'Utilization Rate Tree Map by Cohort (Size is TTM)',
		  	...
		  	function showStaticTooltip(row, size, value) {
    			return '<div style="background:#fd9; padding:10px; border-style:solid">' +
           				...
           				'<b>' + data.getColumnLabel(0) + '</b>: ' + data.getValue(row, 0) +'</span><br>' +
           				'<b>' + data.getColumnLabel(1) + '</b>: ' + data.getValue(row, 1) + '<br>' +
           				'<b>' + data.getColumnLabel(2) + '</b>: ' + data.getValue(row, 2) + '<br>' +
